...
function ask_use_ask_lan
    _ask_write_state --ask_lan $argv
end
...
function get_openai_models
    ...
    if test "$argv[1]" = ollama
        set base_url http://ollama.lan:11434/v1/models
        set api_key ""
        ...
        set base_url http://ask.lan:8014/v1/models
        set api_key ""
...
complete --command ask_use_ask_lan --no-files -a '(get_openai_models ask_lan)'
