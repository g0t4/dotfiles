...
function __fzf_picker --argument-names picker
    ...
    if test -n "$selected_path"
        __fzf_mru_write $picker "$selected_path"

        if test -n "$git_ref"
            # We have git ref, so replace the current token with ref:file
            commandline --current-token -- "$git_ref:$selected_path"
        else
            ...
            commandline --insert -- (string escape -- "$selected_path")
        end
    end
