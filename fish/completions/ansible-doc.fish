...
function __fish_ansible-doc_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        ansible-doc 8>&1 9>&2 1>&9 2>&1
    else
        ansible-doc 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command ansible-doc -f -a '(__fish_ansible-doc_complete)'
