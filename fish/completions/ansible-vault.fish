...
function __fish_ansible-vault_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        ansible-vault 8>&1 9>&2 1>&9 2>&1
    else
        ansible-vault 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command ansible-vault -f -a '(__fish_ansible-vault_complete)'
