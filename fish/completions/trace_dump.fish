...
function __fish_trace_dump_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        trace_dump 8>&1 9>&2 1>&9 2>&1
    else
        trace_dump 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command trace_dump -f -a '(__fish_trace_dump_complete)'
