...
function __fish_ansible-inventory_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        ansible-inventory 8>&1 9>&2 1>&9 2>&1
    else
        ansible-inventory 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command ansible-inventory -f -a '(__fish_ansible-inventory_complete)'
