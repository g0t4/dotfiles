...
function __fish_view_trace_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        view_trace 8>&1 9>&2 1>&9 2>&1
    else
        view_trace 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command view_trace -f -a '(__fish_view_trace_complete)'
