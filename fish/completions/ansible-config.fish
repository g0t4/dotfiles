...
function __fish_ansible-config_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        ansible-config 8>&1 9>&2 1>&9 2>&1
    else
        ansible-config 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command ansible-config -f -a '(__fish_ansible-config_complete)'
