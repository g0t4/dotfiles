...
function __fish_ansible-playbook_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        ansible-playbook 8>&1 9>&2 1>&9 2>&1
    else
        ansible-playbook 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command ansible-playbook -f -a '(__fish_ansible-playbook_complete)'
