...
function __fish_browse_traces_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        browse_traces 8>&1 9>&2 1>&9 2>&1
    else
        browse_traces 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command browse_traces -f -a '(__fish_browse_traces_complete)'
