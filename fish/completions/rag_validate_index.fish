...
function __fish_rag_validate_index_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        rag_validate_index 8>&1 9>&2 1>&9 2>&1
    else
        rag_validate_index 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command rag_validate_index -f -a '(__fish_rag_validate_index_complete)'
