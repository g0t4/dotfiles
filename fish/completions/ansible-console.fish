...
function __fish_ansible-console_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        ansible-console 8>&1 9>&2 1>&9 2>&1
    else
        ansible-console 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command ansible-console -f -a '(__fish_ansible-console_complete)'
