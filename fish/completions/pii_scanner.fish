...
function __fish_pii_scanner_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        pii_scanner 8>&1 9>&2 1>&9 2>&1
    else
        pii_scanner 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command pii_scanner -f -a '(__fish_pii_scanner_complete)'
