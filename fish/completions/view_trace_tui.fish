...
function __fish_view_trace_tui_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        view_trace_tui 8>&1 9>&2 1>&9 2>&1
    else
        view_trace_tui 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command view_trace_tui -f -a '(__fish_view_trace_tui_complete)'
