...
function __fish_ansible-pull_complete
    ...
    set -lx COMP_TYPE
    if set -q _ARC_DEBUG
        ansible-pull 8>&1 9>&2 1>&9 2>&1
    else
        ansible-pull 8>&1 9>&2 1>/dev/null 2>&1
    end
end
complete --command ansible-pull -f -a '(__fish_ansible-pull_complete)'
