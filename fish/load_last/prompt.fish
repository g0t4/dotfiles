...
function prompt_pwd --description 'wes mod - name of the current dir only'
    ...
    if string match -q "$HOME/repos/github/g0t4/private-course-deploying-containerized-application-docker" $PWD
        echo -n -s private-course
        return
    end
    if string match -q "$HOME/repos/github/g0t4/course-deploying-containerized-application-docker" $PWD
        echo -n -s course
        return
    end
