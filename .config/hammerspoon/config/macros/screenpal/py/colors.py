...
timeline_bg_opencv = timeline_bg_color_digital_color_meter + cv_adjust
print(f'{timeline_bg_opencv=}')

# #21253B => R=0x21 (33) G=0x25 (37) B=0x3B (59)  -- DIGITAL COLOR METER
silence_gray_box_color_digital_color_meter = np.array([59, 37, 33])
silence_gray_opencv = silence_gray_box_color_digital_color_meter + cv_adjust
print(f'{silence_gray_opencv=}')
...
if np.array_equal(sample_timeline_bg, timeline_bg_opencv) is False:
    raise ValueError(f"expected {timeline_bg_opencv=} but got {sample_timeline_bg=}")

# y=50, x=1650 (4k original resolution) => silence gray box
sample_gray_box = cv_image[50, 1650]
print(f'{sample_gray_box=}')
if np.array_equal(sample_gray_box, silence_gray_opencv) is False:
    raise ValueError(f"expected {silence_gray_opencv=} but got {sample_gray_box=}")
