return {
...

    {
        ...
        config = function()
            ...
            local half_page = 50
            local a_few = 10
            local scroll_cursor_line = 20
            local keymap = {
                -- scroll full page:
                ["<C-b>"]      = function() neoscroll.ctrl_b({ duration = full_page }) end, -- 450 default
                ["<C-f>"]      = function() neoscroll.ctrl_f({ duration = full_page }) end, -- 450 default
                ["<PageUp>"]   = function() neoscroll.ctrl_b({ duration = full_page }) end, -- not mapped by neoscroll
                ["<PageDown>"] = function() neoscroll.ctrl_f({ duration = full_page }) end, -- not mapped by neoscroll
                --
                -- scroll half page:
                -- TODO this feels like it is not applying? it is still 250 ms even after changing here
                ["<C-u>"]      = function() neoscroll.ctrl_u({ duration = half_page }) end, -- 250 default
                ["<C-d>"]      = function() neoscroll.ctrl_d({ duration = half_page }) end, -- 250 default
                ...
                ["<C-y>"]      = function() neoscroll.scroll(-0.1, { move_cursor = false, duration = a_few }) end, -- 100 default
                ["<C-e>"]      = function() neoscroll.scroll(0.1, { move_cursor = false, duration = a_few }) end, -- 100 default (feels slow)
                -- FYI love move_cursor param! allows me to use original C-e/y design and have a scrollunder variant:
                ["<C-S-y>"]    = function() neoscroll.scroll(-0.1, { move_cursor = true, duration = a_few }) end, -- 100 default
                ["<C-S-e>"]    = function() neoscroll.scroll(0.1, { move_cursor = true, duration = a_few }) end, -- 100 default (feels slow)
                ...
                ["zt"]         = function() neoscroll.zt({ half_win_duration = scroll_cursor_line }) end, -- 250 half_win_duration default
                ["zz"]         = function() neoscroll.zz({ half_win_duration = scroll_cursor_line }) end, -- 250 half_win_duration default
                ["zb"]         = function() neoscroll.zb({ half_win_duration = scroll_cursor_line }) end, -- 250 half_win_duration default, too slow when move is large (top line to bottom = 2x250 = 500ms)
            }
            local modes = { 'n', 'v', 'x' }
