...
abbr ve 'python3.10 -m venv --clear --upgrade-deps'
abbr vedir 'echo $VIRTUAL_ENV'
abbr veinit 'python3.10 -m venv --clear --upgrade-deps .venv && vea' # PRN follow with pip install -r requirements.txt (if req file exists)
abbr veinitr 'python3.10 -m venv --clear --upgrade-deps .venv && vea && pip3 install -r requirements.txt'
# PRN make install requirements.txt conditional on its presence in current dir => i.e. fish abbreviation + function
abbr veinitl 'python3.10 -m venv --clear --upgrade-deps .venv.local && vea'
